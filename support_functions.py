...
"""
##Modules necessary to run all the support function
import numpy as np
...
#This function will check the validity of the date inputs.
    ...
    if proper_format.match(start_date) is None and proper_format.match(end_date) is not None:
        ...
        if k1==1 and k2==0:
            ...
            if start_date==end_date:
                ...
                message='Dates ok'
    return message #message tells the main program what the problem was with the date inputs.

def twitter_scrapper(twitter_username,start_date,end_date): 
#This function will retrieve the tweets of twitter_username between the start_date and the end_date.
    unixdates=[]
    dates=[]
    times=[]
    content=[]
    ...
    if mistake==False:
        ...
        return 'Uwrong'
    #return either an error message when the username does not exist between the selected dates or the scrapped tweets' data.
    
def engagement_metrics(unixdates,dates,reply_count,like_count,retweet_count,quote_count):
#This function returns a (1,4) bar plots with the like, reply, retweet and quote counts for each tweet during the period.    
    ##Plot
    
    def calibration_ax(x_data,y_data): #This function calibrates the figure's axes for the period.
    ...
    return fig #fig being the bar plot in this case.
...
#This function simply search the scrapped data for the 5 tweets with the most likes and return that specific part of the data.   
    ##Arranging data
    if len(like_count)<5:
...
#This function returns a sunburst chart with the engagement metrics and words information.
    #Arranging data
    #Metrics
    ...
    return fig #fig being the sunburst chart in this case
